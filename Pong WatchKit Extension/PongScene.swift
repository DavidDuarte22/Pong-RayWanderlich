...
final class PongScene: SKScene {
  ...
  override func update(_ currentTime: TimeInterval) {
    ...
    // Regardless of why you exit the method, the code ensures the current crown position reflects as previousCrownPosition on the next update.
    defer { previousCrownPosition = newPosition }

    // If there’s no change, you exit the method right away to keep performance as fast as possible
    ...
    // Calculate the amount by which the user scrolled the Digital Crown, storing the paddle’s new location.
    let offset = newPosition - previousCrownPosition
    let y = paddleBeingMoved.position.y + offset

    // Perform bounds checking on the paddle’s location. The scene knows the minimum and maximum y values, so if you continue to turn the Digital Crown, the paddle doesn’t leave the game zone.
    guard minPaddleY ... maxPaddleY ~= y else { return }

    // Everything is copacetic, so you tell the paddle about its new location.
